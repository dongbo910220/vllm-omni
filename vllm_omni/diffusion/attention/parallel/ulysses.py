...
def _all_gather_int_cpu(pg: dist.ProcessGroup, value: int) -> list[int]:
    """All-gather a scalar int across pg using CPU tensors (requires gloo PG)."""
    world_size = dist.get_world_size(pg)
    if world_size == 1:
        return [int(value)]

    t = torch.tensor([int(value)], dtype=torch.int64, device="cpu")
        ...
        if mode == "advanced_uaa":
            if self._sp_group.ring_world_size > 1:
                raise ValueError(
                    ...
                    f"ring_degree={self._sp_group.ring_world_size} is not supported."
                )
            ...
            seq_lens = _all_gather_int_cpu(self._sp_group.cpu_group, local_seq_len)
